...
function deriveTrend(tip: CoachTipOut): { metric: string; metricLabel: string; data: number[] } {
  ...
  return {
    metric: `${sign}${delta}%`,
    metricLabel: tip.severity === "critical" ? "Risque Détecté" : tip.severity === "warning" ? "Écart Performance" : "Tendance Forme",
    data,
  };
}
    ...
    <Panel className="mx-5 mb-4 p-5">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center">
        <div className="flex-1">
          ...
          <h3 className="mt-3.5 text-[14px] font-bold leading-tight" style={{ color: COLORS.heading, fontFamily: FONTS.body }}>
            {tip.title}
          </h3>
        ...
        <div className="flex shrink-0 flex-col items-end gap-2 lg:w-44">
          <div className="text-right">
            <p className="text-[22px] font-black leading-none" style={{ color: COLORS.heading, fontFamily: FONTS.mono }}>
              {trend.metric}
            </p>
            <p className="text-[9px] uppercase tracking-wider mt-1" style={{ color: COLORS.gray50, fontFamily: FONTS.mono }}>
              {trend.metricLabel}
            </p>
          </div>
