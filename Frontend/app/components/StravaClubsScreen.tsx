...
function InviteButton({ firstname }: { firstname: string }) {
	...
	return (
		<button
			...
			className="flex flex-shrink-0 items-center gap-1.5 rounded-xl px-3 py-1.5 text-[10px] font-bold tracking-wider uppercase transition-all"
			style={{
...
function ClubMembersView({ club, onBack }: { club: StravaClubOut; onBack: () => void }) {
	...
	return (
		<div>
			<button
				onClick={onBack}
				className="flex items-center gap-1 px-5 pt-4 pb-2 text-[11px] font-bold tracking-wider uppercase transition-opacity hover:opacity-70"
			...
			<div className="px-5 pb-3">
				<h2 className="text-[17px] leading-tight font-bold" style={{ color: COLORS.heading, fontFamily: FONTS.title }}>
					{club.name}
				</h2>
		...
		<div className="flex flex-col gap-2 px-5 pb-6">
			{clubs.map((club) => (
				<button
					...
					className="glass-panel flex items-center gap-3 rounded-2xl px-4 py-3 text-left transition-opacity hover:opacity-80"
				>
					<div
					...
					>
						{club.profile_medium ? (
							...
							<img src={club.profile_medium} alt="" className="h-full w-full object-cover" />
						) : (
...
export function StravaClubsScreen() {
