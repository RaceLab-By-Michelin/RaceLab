...
export const tiresApi = {
  ...
  getTires: (): Promise<TireSetOut> =>
    api.get<TireSetOut>("/tires").then((r) => r.data),

  /** GET /tires/{wheel} */
  getTire: (wheel: WheelPosition): Promise<TireOut> =>
    api.get<TireOut>(`/tires/${wheel}`).then((r) => r.data),
  ...
  getCatalog: (type?: string): Promise<TireCatalogOut[]> =>
    api
      .get<TireCatalogOut[]>("/tires/catalog", { params: type ? { type } : {} })
      .then((r) => r.data),
  ...
  getWearHistory: (days: 30 | 60 | 90 = 30): Promise<WearHistoryOut> =>
    api
      .get<WearHistoryOut>("/tires/wear-history", { params: { days } })
      .then((r) => r.data),
