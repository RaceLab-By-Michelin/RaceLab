...
export const settingsApi = {
	...
	patchNotifications: (body: NotificationSettingsPatch): Promise<NotificationSettingsOut> =>
		api.patch<NotificationSettingsOut>('/settings/notifications', body).then((r) => r.data),

	// ── Strava ────────────────────────────────────────────────────────────────

	/** GET /settings/strava */
	getStrava: (): Promise<StravaOut> => api.get<StravaOut>('/settings/strava').then((r) => r.data),
	...
	syncStrava: (): Promise<StravaSyncOut> => api.post<StravaSyncOut>('/settings/strava/sync').then((r) => r.data),

	/** DELETE /settings/strava/disconnect */
	disconnectStrava: (): Promise<StravaOut> => api.delete<StravaOut>('/settings/strava/disconnect').then((r) => r.data),
