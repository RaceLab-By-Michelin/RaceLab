...
test.describe("protected app shell", () => {
  ...
  test("renders telemetry data from the API", async ({ page }) => {
    ...
    await expect(page.getByText(/Roue Arri.re/).filter({ visible: true }).first()).toBeVisible();
  });
  ...
  test("navigates across critical protected screens", async ({ page }) => {
    ...
    for (const screen of screens) {
      ...
      await expect(page.getByRole("heading", { name: /usure pr.dictive/i })).toBeVisible();
      await expect(appNav.getByRole("button", { name: /Donn.es/i }).filter({ visible: true }).first()).toHaveAttribute("aria-current", "page");

      const navButton = appNav.getByRole("button", { name: new RegExp(screen.nav, "i") }).filter({ visible: true }).first();
